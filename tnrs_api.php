...
if ( $mode=="parse" || $mode=="resolve" || $mode=='syn' || $mode=="" ) { // BEGIN mode_if
	...
	if ( $mode=='syn' ) {	// BEGIN synonym lookup
		...
		$acc_url = $results_array[0]['Accepted_name_url'];
		...
		$sql="
		SELECT DISTINCT 
		'$name_submitted' AS submitted_name, 
		'$matched_nameauth' AS matched_nameWithAuthor,
		'$name_matched_status' AS matched_taxonomicStatus,
		acc_scientificNameWithAuthor AS accepted_nameWithAuthor, 
		acc_sourceName AS accepted_source, 
		acc_nameSourceUrl AS accepted_nameUrl,
		syn_scientificNameWithAuthor AS syn_nameWithAuthor, 
		...
		syn.acceptance AS taxonomicStatus, s.sourceName as syn_sourceName,
		ns.nameSourceUrl AS syn_nameSourceUrl,
		acc.scientificNameWithAuthor AS directSynOfNameWithAuthor
		...
		AND s.sourceName=@source
		) AS acc
		WHERE s.sourceName='$src'
		AND syn.acceptedNameID=acc.nameID
		UNION ALL
		-- Synonyms of accepted name
		SELECT parent.nameID AS acc_nameID,
		parent.scientificNameWithAuthor AS acc_scientificNameWithAuthor,
		'$src' AS acc_sourceName, '$acc_url' AS acc_nameSourceUrl,
		child.nameID AS syn_nameID, child.scientificNameWithAuthor AS syn_scientificNameWithAuthor,
		syn.acceptance AS taxonomicStatus, 
		'$src' AS syn_sourceName, nameSourceUrl AS syn_nameSourceUrl,
		accChildName.scientificNameWithAuthor AS directSynOfNameWithAuthor
		FROM classification c JOIN name parent ON c.parentNameID=parent.nameID 
		JOIN name child ON c.nameID=child.nameID
		JOIN source s ON c.sourceID=s.sourceID
		JOIN name_source ns
		ON child.nameID=ns.nameID AND s.sourceID=ns.sourceID
		JOIN synonym syn ON c.nameID=syn.nameID AND c.sourceID=syn.sourceID
		LEFT JOIN name AS accChildName ON syn.acceptedNameID=accChildName.nameID
		JOIN classification c2 ON syn.acceptedNameID=c2.nameID
		JOIN name AS accChildParentName ON accChildParentName.nameID=c2.parentNameID
		WHERE parent.scientificNameWithAuthor='$acc_nameauth'
		...
		// Handle empty response due to no synonyms
		if ( count($results_array)==0 ) {
			...
			'$matched_nameauth' AS matched_nameWithAuthor,
			'$name_matched_status' AS matched_taxonomicStatus,
			'$acc_nameauth' AS accepted_nameWithAuthor, 
			'$src' AS accepted_source, 
			'$acc_url' AS accepted_nameUrl,
			'[No synonyms found]' AS syn_nameWithAuthor, 
			...
			// Query again to generate single element $results_array containing 
			// submitted, matched & accepted name + '[No synonyms found]'
			include("qy_db.php"); 
		}
